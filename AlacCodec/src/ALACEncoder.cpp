        ...
        HEADER SPECIFICATION
                ...
                For every segment we adopt the following header:
                        ...
                        (numV) shorts			(signed DP coefficients for V channel)
                                 ---After this come the shift-off bytes for (>= 24)-bit data (n-byte shift) if
   indicated---
                ...
                not even be sent.  The currently defined flags format is:
                        ...
                        where		0 	= reserved, must be 0
                                                p	= 1-bit field "partial frame" flag indicating 32-bit frame
   length follows this byte
                                                ss	= 2-bit field indicating "number of shift-off bytes ignored by
   compression"
...
*/
int32_t ALACEncoder::EncodeStereo(BitBuffer *bitstream, void *inputBuffer, uint32_t stride, uint32_t channelIndex,
                                  uint32_t numSamples) {
  BitBuffer workBits;
  BitBuffer startBits =
  ...
  RequireAction((mBitDepth == 16) || (mBitDepth == 20) || (mBitDepth == 24) || (mBitDepth == 32),
  ...
  for (mixRes = 0; mixRes <= maxRes; mixRes++) {
    ...
    switch (mBitDepth) {
      ...
      case 24:
        ...
        mix24((uint8_t *)inputBuffer, stride, mMixBufferU, mMixBufferV, numSamples / dilate, mixBits, mixRes,
              mShiftBufferUV, bytesShifted);
        break;
      case 32:
        // includes extraction of shifted-off bytes
        mix32((int32_t *)inputBuffer, stride, mMixBufferU, mMixBufferV, numSamples / dilate, mixBits, mixRes,
              mShiftBufferUV, bytesShifted);
        break;
    }
  ...
  switch (mBitDepth) {
    ...
    case 24:
      ...
      mix24((uint8_t *)inputBuffer, stride, mMixBufferU, mMixBufferV, numSamples, mixBits, mixRes, mShiftBufferUV,
            bytesShifted);
      break;
    case 32:
      // also extracts the shifted off bytes into the shift buffers
      mix32((int32_t *)inputBuffer, stride, mMixBufferU, mMixBufferV, numSamples, mixBits, mixRes, mShiftBufferUV,
            bytesShifted);
      break;
  }
  ...
  if (doEscape == false) {
    ...
    for (index = 0; index < numU; index++) BitBufferWrite(bitstream, coefsU[numU - 1][index], 16);

    BitBufferWrite(bitstream, (mode << 4) | DENSHIFT_DEFAULT, 8);
    BitBufferWrite(bitstream, (pbFactor << 5) | numV, 8);
    for (index = 0; index < numV; index++) BitBufferWrite(bitstream, coefsV[numV - 1][index], 16);
...
*/
int32_t ALACEncoder::EncodeStereoFast(BitBuffer *bitstream, void *inputBuffer, uint32_t stride, uint32_t channelIndex,
                                      uint32_t numSamples) {
...
*/
int32_t ALACEncoder::EncodeMono(BitBuffer *bitstream, void *inputBuffer, uint32_t stride, uint32_t channelIndex,
                                uint32_t numSamples) {
  BitBuffer startBits =
      *bitstream;  // squirrel away copy of current state in case we need to go back and do an escape packet
  AGParamRec agParams;
  ...
  if (doEscape == false) {
    ...
    // if shift active, write the interleaved shift buffers
    if (bytesShifted != 0) {
      for (index = 0; index < numSamples; index++) BitBufferWrite(bitstream, mShiftBufferUV[index], shift);
    }
  ...
  if (doEscape == true) {
    ...
    switch (mBitDepth) {
      case 16:
        input16 = (int16_t *)inputBuffer;
        for (index = 0; index < (numSamples * stride); index += stride) BitBufferWrite(bitstream, input16[index], 16);
        break;
      case 20:
        // convert 20-bit data to 32-bit for simplicity
        copy20ToPredictor((uint8_t *)inputBuffer, stride, mMixBufferU, numSamples);
        for (index = 0; index < numSamples; index++) BitBufferWrite(bitstream, mMixBufferU[index], 20);
        break;
      case 24:
        // convert 24-bit data to 32-bit for simplicity
        copy24ToPredictor((uint8_t *)inputBuffer, stride, mMixBufferU, numSamples);
        for (index = 0; index < numSamples; index++) BitBufferWrite(bitstream, mMixBufferU[index], 24);
        break;
      case 32:
        input32 = (int32_t *)inputBuffer;
        for (index = 0; index < (numSamples * stride); index += stride) BitBufferWrite(bitstream, input32[index], 32);
        break;
    }
#if VERBOSE_DEBUG
...
int32_t ALACEncoder::Encode(AudioFormatDescription theInputFormat, AudioFormatDescription theOutputFormat,
    ...
    for (channelIndex = 0; channelIndex < theInputFormat.mChannelsPerFrame;) {
      ...
      switch (tag) {
        ...
        case ID_CPE:
          ...
          status =
              this->EncodeStereo(&bitstream, inputBuffer, theInputFormat.mChannelsPerFrame, channelIndex, numFrames);
...
#if VERBOSE_DEBUG
  {
    ...
    if ((bytesLeft > 20) && ((bytesLeft & 0x4u) != 0)) AddFiller(&bitstream, bytesLeft);
  }
#endif
...
int32_t ALACEncoder::Finish() {
  /*	// finalize bit rate statistics
          if ( mSampleSize.numEntries != 0 )
                  mAvgBitRate = (uint32_t)( (((float)mTotalBytesGenerated * 8.0f) / (float)mSampleSize.numEntries) *
     ((float)mSampleRate / (float)mFrameSize) );
          else
                  mAvgBitRate = 0;
  */
  return ALAC_noErr;
...
int32_t ALACEncoder::InitializeEncoder(AudioFormatDescription theOutputFormat) {
  ...
  for (uint32_t index = 0; index < kALACMaxChannels; index++) mLastMixRes[index] = kDefaultMixRes;
  ...
  RequireAction((mMixBufferU != nil) && (mMixBufferV != nil) && (mPredictorU != nil) && (mPredictorV != nil) &&
...
void ALACEncoder::GetSourceFormat(const AudioFormatDescription *source, AudioFormatDescription *output) {
  ...
  /*
  MakeUncompressedAudioFormat( mNumChannels, (float) mOutputSampleRate, mBitDepth, kAudioFormatFlagsNativeIntegerPacked,
  output );
   */
}
...
static void AddFiller(BitBuffer *bits, int32_t numBytes) {
  ...
  if (tag == ID_FIL) {
    ...
    for (index = 0; index < (numBytes - 1); index++) BitBufferWrite(bits, 0xa5, 8);  // fill_byte = b10100101 = 0xa5
  } else {
