...
/*
        Constructor
*/
ALACDecoder::ALACDecoder() : mMixBufferU(nil), mMixBufferV(nil), mPredictor(nil), mShiftBuffer(nil) {
...
int32_t ALACDecoder::Init(void *inMagicCookie, uint32_t inMagicCookieSize) {
  ...
  if (theCookieBytesRemaining >= sizeof(ALACSpecificConfig)) {
    ...
    RequireAction((mMixBufferU != nil) && (mMixBufferV != nil) && (mPredictor != nil), status = kALAC_MemFullError;
                  goto Exit;);
  } else {
...
/*
        ...
        - the decoded samples are interleaved into the output buffer in the order they arrive in
          the bitstream
*/
int32_t ALACDecoder::Decode(BitBuffer *bits, uint8_t *sampleBuffer, uint32_t numSamples, uint32_t numChannels,
  ...
  RequireAction((bits != nil) && (sampleBuffer != nil) && (outNumSamples != nil), status = kALAC_ParamError;
                goto Exit;);
    ...
    switch (tag) {
      ...
      case ID_LFE: {
        ...
        if (escapeFlag == 0) {
          ...
          // decompress
          set_ag_params(&agParams, mConfig.mb, (pb * pbFactorU) / 4, mConfig.kb, numSamples, numSamples,
                        mConfig.maxRun);
        ...
        if (bytesShifted != 0) {
          ...
          for (i = 0; i < numSamples; i++) mShiftBuffer[i] = (uint16_t)BitBufferRead(&shiftBits, (uint8_t)shift);
        }
        ...
        switch (mConfig.bitDepth) {
          case 16:
            ...
            for (i = 0, j = 0; i < numSamples; i++, j += numChannels) out16[j] = (int16_t)mMixBufferU[i];
            break;
          case 20:
      ...
      case ID_CPE: {
        ...
        if (escapeFlag == 0) {
          ...
          set_ag_params(&agParams, mConfig.mb, (pb * pbFactorV) / 4, mConfig.kb, numSamples, numSamples,
                        mConfig.maxRun);
        ...
        switch (mConfig.bitDepth) {
          ...
          case 24:
            out24 = (uint8_t *)sampleBuffer + (channelIndex * 3);
            unmix24(mMixBufferU, mMixBufferV, out24, numChannels, numSamples, mixBits, mixRes, mShiftBuffer,
                    bytesShifted);
            break;
          case 32:
            out32 = &((int32_t *)sampleBuffer)[channelIndex];
            unmix32(mMixBufferU, mMixBufferV, out32, numChannels, numSamples, mixBits, mixRes, mShiftBuffer,
                    bytesShifted);
            break;
        }
...
static void Zero16(int16_t *buffer, uint32_t numItems, uint32_t stride) {
  if (stride == 1) {
    ...
    for (uint32_t index = 0; index < (numItems * stride); index += stride) buffer[index] = 0;
  }
}
