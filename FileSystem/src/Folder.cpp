...
namespace FileSystem
{
	...
	wstring Folder::GetName() const
	{
		return wstring(m_folder->Name->Data());
	}

	wstring Folder::GetFullPath() const
	{
		return wstring(m_folder->Path->Data());
	}
	...
	shared_ptr<IFile> Folder::CreateNewFile(const wstring& fileName)
	{
		...
		try
		{
			auto result = make_shared<File>(create_task(m_folder->CreateFileAsync(winrt_fileName, Windows::Storage::CreationCollisionOption::OpenIfExists)).get());
			return result;
		}
		catch (Platform::COMException^ ex)
		{
	...
	shared_ptr<Util::WorkBuffer<shared_ptr<IFile>>> Folder::FindFilesWithExtensions(vector<wstring> extensions)
	{
			...
			.then([buffer, ext](IVectorView<StorageFolder^>^ allFolders)
		{
			...
			for (const auto& folder : allFolders)
			{
				...
				{
					vector<shared_ptr<IFile>> results;
					transform(begin(files), end(files), back_inserter(results), [](StorageFile^ file)
					{
						return make_shared<File>(file);
					});
