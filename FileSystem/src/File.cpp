...
File::File(Windows::Storage::StorageFile ^ file) : m_file(file) { ARC_ASSERT(file != nullptr); }
...
std::wstring File::GetFullName() const { return std::wstring(m_file->Name->Data()); }

std::wstring File::GetFullPath() const { return std::wstring(m_file->Path->Data()); }
...
void File::ReadFromFile(std::vector<unsigned char>& buffer, unsigned int length /* = 0*/,
                        unsigned long long startPosition /* = 0*/) const {
  ...
  if (length > 0) {
    ARC_ASSERT_MSG(startPosition < randomAccessStream->Size,
                   "Attempting to start reading from a point beyond the end of the file!");
    ...
    auto task =
        create_task(inputStream->ReadAsync(nativeBuffer, length, Windows::Storage::Streams::InputStreamOptions::None));
    task.wait();
  }
}
...
std::vector<unsigned char> File::GetThumbnail(bool allowIcon) const {
  ...
  try {
    auto thumbnailStream =
        create_task(m_file->GetThumbnailAsync(Windows::Storage::FileProperties::ThumbnailMode::SingleItem)).get();
...
Windows::Storage::StorageFile ^ File::GetRawHandle() const { return m_file; }
#endif
}