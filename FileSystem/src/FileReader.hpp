...
FileReader<IFilePtr>::FileReader(const IFilePtr file, unsigned int bufferSize)
    : IFileReader(), m_file(file), m_bufferSize(bufferSize), m_bytesRead(0), m_bytesReadFromFile(0) {
...
std::vector<unsigned char> FileReader<IFilePtr>::GetBytes(unsigned int count) {
  ...
  if (count <= availableBytes) {
    ...
    ARC_ASSERT_MSG(remainingBytes <= bytesRemainingInFile,
                   "Warning: attempting to read more bytes than are in this file! Returned buffer will be smaller than "
                   "requested!");
    ...
    if (bytesToReadFromFile > 0) {
      ...
      m_endPosition = m_buffer.begin() + std::min(bytesToStoreInBuffer,
                                                  Util::SafeIntCast<decltype(bytesToStoreInBuffer)>(m_buffer.size()));
