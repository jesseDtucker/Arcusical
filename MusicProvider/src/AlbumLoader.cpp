...
struct iequal_to
{
	...
	{
		return boost::algorithm::iequals(a, b, locale());
	}
};
...
static vector<Album> CreateNewAlbums(AlbumLookupByAlbumName albums, SongLookupByAlbumName songGroups, const IAlbumToSongMapper* mapper);
static vector<Album> CreateModifiedAlbums(AlbumLookupByAlbumName albums, SongLookupByAlbumName songGroups);

static vector<pair<uuid, const Album*>> DetermineMissingSongs(const AlbumCollection& existingAlbums, const SongCollection& songs);
static vector<pair<const Album*, vector<uuid>>> GroupDeletedAlbums(const vector<pair<uuid, const Album*>>& missingSongs);
