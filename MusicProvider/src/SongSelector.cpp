...
namespace Arcusical
{
	...
	{
		SongSelector::SongSelector(LocalMusicStore::LocalMusicCache* cache)
			: m_cache(cache)
		{
		...
		vector<Model::Song> SongSelector::GetFromSameArtist(const Model::Song& targetSong, function<bool(const Model::Song&)> filter)
		{
			auto songs = m_cache->GetLocalSongs();
			vector<Model::Song> result;
