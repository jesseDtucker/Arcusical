...
HRESULT ALACMFTDecoder::GetStreamLimits(DWORD *pdwInputMinimum, DWORD *pdwInputMaximum, DWORD *pdwOutputMinimum,
                                        DWORD *pdwOutputMaximum) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;

  if ((pdwInputMinimum == nullptr) || (pdwInputMaximum == nullptr) || (pdwOutputMinimum == nullptr) ||
...
HRESULT ALACMFTDecoder::GetStreamCount(DWORD *pcInputStreams, DWORD *pcOutputStreams) {
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::GetStreamIDs(DWORD dwInputIDArraySize, DWORD *pdwInputIDs, DWORD dwOutputIDArraySize,
                                     DWORD *pdwOutputIDs) {
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::GetInputStreamInfo(DWORD dwInputStreamID, MFT_INPUT_STREAM_INFO *pStreamInfo) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::GetOutputStreamInfo(DWORD dwOutputStreamID, MFT_OUTPUT_STREAM_INFO *pStreamInfo) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
  ...
  pStreamInfo->dwFlags =
      MFT_OUTPUT_STREAM_WHOLE_SAMPLES | MFT_OUTPUT_STREAM_FIXED_SAMPLE_SIZE | MFT_OUTPUT_STREAM_PROVIDES_SAMPLES;
...
HRESULT ALACMFTDecoder::GetInputStreamAttributes(DWORD dwInputStreamID, IMFAttributes **ppAttributes) {
  // CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::GetOutputStreamAttributes(DWORD dwOutputStreamID, IMFAttributes **ppAttributes) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::AddInputStreams(DWORD cStreams, DWORD *adwStreamIDs) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::GetInputAvailableType(DWORD dwInputStreamID, DWORD dwTypeIndex, IMFMediaType **ppType) {
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::GetOutputAvailableType(DWORD dwOutputStreamID, DWORD dwTypeIndex,  // 0-based
                                               IMFMediaType **ppType) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
// Name: SetInputType
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::SetInputType(DWORD dwInputStreamID,
...
// Name: SetOutputType
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::SetOutputType(DWORD dwOutputStreamID,
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::GetInputCurrentType(DWORD dwInputStreamID, IMFMediaType **ppType) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::GetOutputCurrentType(DWORD dwOutputStreamID, IMFMediaType **ppType) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::GetInputStatus(DWORD dwInputStreamID, DWORD *pdwFlags) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::SetOutputBounds(LONGLONG /*hnsLowerBound*/, LONGLONG /*hnsUpperBound*/) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::ProcessEvent(DWORD /*dwInputStreamID*/, IMFMediaEvent * /*pEvent */) {
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
// Name: ProcessMessage
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::ProcessMessage(MFT_MESSAGE_TYPE eMessage, ULONG_PTR ulParam) {
  SYNC_LOCK;
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::ProcessInput(DWORD dwInputStreamID, IMFSample *pSample, DWORD dwFlags) {
  SYNC_LOCK;
  CHECK_UNLOCKED;
  CHECK_SHUTDOWN;
  CHECK_NULL(pSample);
...
//-------------------------------------------------------------------
HRESULT ALACMFTDecoder::ProcessOutput(DWORD dwFlags, DWORD cOutputBufferCount,
  ...
  ARC_ASSERT_MSG(pOutputSamples->pSample == nullptr,
                 "We are allocating the samples, there should not be any provided samples!");
...
HRESULT ALACMFTDecoder::CreateOutputType(Microsoft::WRL::ComPtr<IMFMediaType> &spOutputType) {
  ...
  try {
    ...
    hr = spOutputType->SetUINT32(MF_MT_AUDIO_AVG_BYTES_PER_SECOND,
                                 mediaInfo.samplesPerSecond * mediaInfo.bitsPerSample * mediaInfo.channelCount / 8);
    ARC_ThrowIfFailed(hr);
...
STDMETHODIMP ALACMFTDecoder::QueueEvent(MediaEventType met, REFGUID guidExtendedType, HRESULT hrStatus,
...
ComPtr<IMFSample> ALACMFTDecoder::DecodeBuffer(const ComPtr<IMFSample> &pSample) {
  ...
  // scope is to unlock the media buffers as soon as we are done with them
  {
    ...
    ARC_ASSERT(outputBufferLength ==
               outputBufferLength);  // one is what we told it to be, the other is what it said it is

    uint32_t numSamplesOut = 0;
    auto decodeResult = m_decoder.Decode(&bitsWrapper, outBuffer, mediaInfo.alacBox->GetSamplePerFrame(),
                                         mediaInfo.channelCount, &numSamplesOut);
