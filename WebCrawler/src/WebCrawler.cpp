...
namespace Web {
	...
	Crawler::Crawler(std::function<bool(const wstring&)> filter)
		: m_linkFilter(filter)
	{
	...
	void LoadFromCached(const wstring& addr, unordered_set<wstring>& checkedUrls, unordered_set<wstring>& uncheckedUrls, unordered_set<wstring>& allUrls)
	{
	...
	void SaveToCache(const wstring& addr, const unordered_set<wstring>& checkedUrls, const unordered_set<wstring>& uncheckedUrls, const unordered_set<wstring>& allUrls)
	{
		auto cachePath = GetCachedPath(addr);
