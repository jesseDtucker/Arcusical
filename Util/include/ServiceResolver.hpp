...
void ServiceResolver<T>::RegisterSingleton(std::shared_ptr<T> singleton) {
  ARC_ASSERT_MSG(s_singleton == nullptr,
                 "A service of name " << T::ServiceName
...
void ServiceResolver<T>::RegisterUrn(const std::string& urn, std::shared_ptr<T> specificInstance) {
  ARC_ASSERT_MSG(s_serviceMap.find(urn) == s_serviceMap.end(), "A urn of name : " << urn << " for the service " << T
                 : ServiceName << " has already been registered! New Registration will stomp old registration!");
  s_serviceMap[urn] = specificInstance;
}

template <typename T>
typename ServiceResolver<T>::ServiceRef ServiceResolver<T>::ResolveService() {
  ARC_ASSERT_MSG(s_singleton != nullptr, "Attempting to access " << T::ServiceName
                                                                 << " when one hasn't been registered!");
  return s_singleton;
}

template <typename T>
typename ServiceResolver<T>::ServiceRef ServiceResolver<T>::ResolveUrn(const std::string& urn) {
  ARC_ASSERT_MSG(s_serviceMap.find(urn) != s_serviceMap.end(),
                 "Attempted to access urn : " << urn << " for service " << T::ServiceName
...
void ServiceResolver<T>::DeregisterSingleton() {
  ARC_ASSERT_MSG(s_singleton != nullptr, "Attempted to deregister service of name "
                                             << T::ServiceName << " when there was such service registered!");
  s_singleton = nullptr;
}
...
void ServiceResolver<T>::DeregisterUrn(const std::string& urn) {
  ARC_ASSERT_MSG(s_serviceMap.find(urn) != s_serviceMap.end(),
                 "Attempted to deregister urn: " << urn << " for service " << T::ServiceName
                                                 << " when no such urn is registered!");
