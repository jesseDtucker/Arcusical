...
namespace Arcusical { namespace MPEG4 {
	...
	void Esds::ParseTags(Util::Stream& stream, int bytesToRead)
	{
		...
		*/
		// hax
		// ARC_ASSERT_MSG(bytesToRead == 0, "Esds box read too far! It should read precisly the number of bytes it requires!");
	}
	...
	std::ostream& operator << (std::ostream& outStream, DecoderConfigDescription& configDescriptor)
	{
		...
		outStream << " Stream Type: " << static_cast<int>(configDescriptor.m_streamType);	//cast is to ensure correct display
	...
	uint8_t Esds::GetVersion()
	{
		return m_version;
	}

	uint32_t Esds::GetFlags()
	{
		return m_flags;
	}

	std::shared_ptr<ES_Description> Esds::GetES_Description()
	{
		return m_esDescription;
	}

	std::shared_ptr<DecoderConfigDescription> Esds::GetDecoderConfigDescription()
	{
		return m_decoderConfigDescription;
	}

	std::shared_ptr<DecoderSpecificInfo> Esds::GetDecoderSpecificInfo()
	{
		return m_decoderSpecificInfo;
	}

	std::shared_ptr<SL_ConfigDescription> Esds::GetSlConfigDescription()
	{
		return m_slConfigDescription;
	}
