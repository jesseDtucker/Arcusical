...
namespace Arcusical { namespace MPEG4 {
	...
	void Alac::PrintBox(std::ostream& outStream, int depth)
	{
		...
		outStream << tabs << "\tCompatible Version: " << static_cast<int32_t>(m_compatibleVersion) << std::endl;	//cast is needed to assume correct display
	...
	uint16_t Alac::GetNumChannels()
	{
		return m_numChannels;
	}

	uint16_t Alac::GetSampleSize()
	{
		return m_sampleSize;
	}

	uint32_t Alac::GetSampleRate()
	{
		return m_sampleRate;
	}

	uint32_t Alac::GetSamplePerFrame()
	{
		return m_samplePerFrame;
	}

	uint8_t Alac::GetCompatibleVersion()
	{
		return m_compatibleVersion;
	}

	uint32_t Alac::GetMaxFrameSize()
	{
		return m_maxFrameSize;
	}

	uint32_t Alac::GetAvgBitRate()
	{
		return m_avgBitRate;
	}

	uint32_t Alac::GetVersion()
	{
		return m_version;
	}

	uint16_t Alac::GetIndex()
	{
		return m_index;
	}

	AlacChannelLayout Alac::GetChannelLayout()
	{
