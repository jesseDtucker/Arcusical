...
// public
Box::Box() : m_bodySize(0), m_boxName(nullptr), m_fullSize(0) {}
...
// public
uint64_t Box::GetSize() { return m_bodySize; }

// public
std::vector<std::shared_ptr<Box>>& Box::GetChildren() { return m_children; }
...
void Box::ParseBox(uint64_t size, Util::Stream& stream, uint32_t headerSize) {
  ...
  ARC_ASSERT_MSG(bytesReadByBox == m_bodySize, "Box did not read all of the bytes in its box! Bytes read : "
                                                   << bytesReadByBox << " / " << m_bodySize);
