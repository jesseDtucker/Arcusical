...
namespace Arcusical { namespace MPEG4 {
	...
	Tkhd::Tkhd() :	m_version(0), m_flags(0), m_creationTime(0), m_modificationTime(0),
					m_trackID(0), m_duration(0), m_layer(0), m_alternateGroup(0), m_volume(0),
	...
	uint8_t Tkhd::GetVersion()
	{
		return m_version;
	}

	//TODO: break this up into the flags values!
	uint32_t Tkhd::GetFlags()
	{
		return m_flags;
	}
	...
	uint64_t Tkhd::GetAlternateGroup()
	{
		return m_alternateGroup;
	}

	float Tkhd::GetVolume()
	{
		return m_volume;
	}

	std::vector<uint32_t> Tkhd::GetMatrix()
	{
		return m_matrix;
	}
